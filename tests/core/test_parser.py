...
def test_parse_835_minimal(edi_835_minimal_file, edi_835_minimal_json_file):
    """Test parsing a minimal 835 EDI file with only required segments."""
    parser = EdiParser(edi_string=open(edi_835_minimal_file).read(), schema_path="packages/core/schemas/x12/835.json")
    edi_root = parser.parse()
    emitter = EdiEmitter(edi_root)
    json_output = emitter.to_json(pretty=True)
...
def test_parse_835_multiple_claims(edi_835_multiple_claims_file, edi_835_multiple_claims_json_file):
    """Test parsing an 835 EDI file with multiple claims and adjustments."""
    parser = EdiParser(edi_string=open(edi_835_multiple_claims_file).read(), schema_path="packages/core/schemas/x12/835.json")
    edi_root = parser.parse()
    emitter = EdiEmitter(edi_root)
    json_output = emitter.to_json(pretty=True)
...
def test_parse_835_no_payer_payee(edi_835_no_payer_payee_file, edi_835_no_payer_payee_json_file):
    """Test parsing an 835 EDI file without payer/payee information."""
    parser = EdiParser(edi_string=open(edi_835_no_payer_payee_file).read(), schema_path="packages/core/schemas/x12/835.json")
    edi_root = parser.parse()
    emitter = EdiEmitter(edi_root)
    json_output = emitter.to_json(pretty=True)
...
def test_parser_handles_empty_string():
    ...
    parser = EdiParser(edi_string="", schema_path="packages/core/schemas/x12/835.json")
    edi_root = parser.parse()
    emitter = EdiEmitter(edi_root)
    json_output = emitter.to_json(pretty=True)
...
def test_parser_handles_malformed_segments():
    """Test parser behavior with malformed segments."""
    malformed_edi = "ISA*00*          *00*          *ZZ*TEST*ZZ*TEST*230101*0000*U*00501*1*0*P*:~INVALID_SEGMENT~"
    parser = EdiParser(edi_string=malformed_edi, schema_path="packages/core/schemas/x12/835.json")
    edi_root = parser.parse()
...
def test_numeric_formatting():
    ...
    edi_content = """ISA*00*          *00*          *ZZ*TEST*ZZ*TEST*230101*0000*U*00501*1*0*P*:~
...
IEA*1*1~"""
    ...
    parser = EdiParser(edi_string=edi_content, schema_path="packages/core/schemas/x12/835.json")
    edi_root = parser.parse()
    emitter = EdiEmitter(edi_root)
...
def test_date_formatting():
    ...
    edi_content = """ISA*00*          *00*          *ZZ*TEST*ZZ*TEST*230101*1230*U*00501*1*0*P*:~
...
def test_multiple_reference_numbers():
